...
export function PDPClient({ product, relatedProduct }: PDPClientProps) {
  ...
  // Show sticky ATC when the main button scrolls out of view
  useEffect(() => {
    if (!atcBtnRef.current) return
    const obs = new IntersectionObserver(([e]) => setStickyVisible(!e.isIntersecting), {
      threshold: 0,
  ...
  const accordionItems = [
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2 text-[13px] text-[#6B6B6B] leading-relaxed">
          <p>Orders processed within 1–3 business days. Shipping costs and delivery times depend on destination and selected method — calculated at checkout.</p>
          <p>Domestic: approximately 2–7 business days. International: 5–14+ business days. Delivery estimates are not guaranteed.</p>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2 text-[13px] text-[#6B6B6B] leading-relaxed">
          <p>Returns accepted for store credit on unworn, unwashed items with tags attached, within our return window. Customer is responsible for return shipping unless item arrives faulty or incorrect.</p>
  ...
  const atcLabel = soldOut ? 'Sold Out' : addState === 'added' ? 'Added ✓' : addState === 'loading' ? 'Adding…' : selectedSize ? `Add to Bag — ${formatPrice(product.price)}` : 'Select a Size'

  return (
    <>
      {/* Breadcrumb */}
      <nav aria-label="Breadcrumb" className="container-kvrn pt-[calc(36px+56px+16px)] pb-3">
        <ol className="flex items-center gap-2 text-[11px] text-[#9B9B9B] tracking-wide">
      ...
      <div className="container-kvrn pb-24">
          ...
          <div className="space-y-5">
            ...
            <div ref={sizePanelRef}>
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/support/size-guide" className="text-[11px] text-[#9B9B9B] hover:text-[#1A1A1A] underline underline-offset-2 transition-colors">
                  Size guide
                </Link>
              </div>
              <SizeSelector
            ...
            {/* Trust signals — no tax line */}
            <ul className="space-y-2" aria-label="Order assurances">
              {[
                'Quality checked before shipment',
                'Secure checkout',
                'Easy support if something arrives wrong',
              ].map(line => (
        ...
        {/* Construction section — centered, fixed alignment */}
        {product.features.length > 0 && (
          <section className="mt-20 md:mt-28" aria-labelledby="construction-heading">
            <div className="text-center mb-8">
              <p className="text-[11px] font-light tracking-[0.1em] uppercase text-[#9B9B9B]" id="construction-heading">
                The Construction
              </p>
            </div>
            <div className={cn(
        ...
        {/* Complete the Set */}
        {relatedProduct && (
          <section className="mt-16 md:mt-24 pt-12 border-t border-[#E8E5E0]" aria-labelledby="set-heading">
            <p className="text-[11px] font-light tracking-[0.1em] uppercase text-[#9B9B9B] mb-8" id="set-heading">
              Complete the Set
            </p>
      ...
      {/* Sticky impulse ATC — centered, minimal */}
      <div
        className={cn(
          'fixed bottom-0 left-0 right-0 z-[200]',
          'bg-[#F9F8F6]/96 backdrop-blur-sm border-t border-[#E8E5E0]',
          'transition-transform duration-300',
          stickyVisible ? 'translate-y-0' : 'translate-y-full'
        )}
        aria-hidden={!stickyVisible}
      >
        <div className="container-kvrn py-3 flex items-center gap-4 max-w-[540px] mx-auto">
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-light truncate">{product.name}</p>
            {selectedSize && (
              <p className="text-[11px] text-[#9B9B9B]">{selectedColor.name} / {selectedSize}</p>
            )}
          </div>
          <Button
          ...
          >
            {soldOut ? 'Sold Out' : addState === 'added' ? 'Added ✓' : `${formatPrice(product.price)} — Add`}
          </Button>
        </div>
      </div>
    </>
