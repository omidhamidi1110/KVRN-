...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      var bt = c(356).Buffer;
      let bu = process.env.ADMIN_SECRET ?? "change-this-before-launch";
      function bv(a10) {
        let { pathname: b2 } = a10.nextUrl;
        if (b2.startsWith("/admin")) {
          let c2 = a10.cookies.get("kvrn_admin")?.value, d2 = a10.headers.get("authorization");
          if (c2 === bu) return S.next();
          if (d2?.startsWith("Basic ")) try {
            let [, a11] = bt.from(d2.slice(6), "base64").toString().split(":");
            if (a11 === bu) return S.next();
          } catch {
          }
          let e2 = new URL("/admin/login", a10.url);
          return e2.searchParams.set("return", b2), S.redirect(e2);
        }
        return S.next();
      }
      let bw = { matcher: ["/admin/:path*"] };
      Object.values({ NOT_FOUND: 404, FORBIDDEN: 403, UNAUTHORIZED: 401 });
      let bx = { ...f }, by = bx.middleware || bx.default, bz = "/middleware";
